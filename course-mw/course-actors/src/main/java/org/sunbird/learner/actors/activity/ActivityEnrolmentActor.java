...
public class ActivityEnrolmentActor extends BaseActor {
    ...
    private void enrollActivity(Request actorMessage) throws Exception {
        ...
        if (userIds == null || userIds.isEmpty()) {
            throw new ProjectCommonException(
                    ...
                    "userIds list cannot be null or empty",
        ...
        ActivityBatch activityBatch = activityBatchDao.readById(activityId, batchId, actorMessage.getRequestContext());
        ...
        // Validate that the batch belongs to the correct activityType
        if (!activityType.equals(activityBatch.getActivityType())) {
            throw new ProjectCommonException(
                    ResponseCode.invalidRequestData.getErrorCode(),
                    "Activity type mismatch. Expected: " + activityType + ", Found: " + activityBatch.getActivityType(),
                    ResponseCode.CLIENT_ERROR.getResponseCode());
        }
        ...
        for (String userId : userIds) {
            ...
            ActivityUserEnrolment existingEnrollment = activityEnrollmentDao.read(actorMessage.getRequestContext(), userId, activityId, activityType, batchId);
        ...
        if (!enrollmentDataList.isEmpty()) {
            activityEnrollmentDao.batchInsert(actorMessage.getRequestContext(), enrollmentDataList);
        }

        // Generate event for CF activities (only for new enrollments)
        if ("CF".equalsIgnoreCase(activityType) && !enrollmentDataList.isEmpty()) {
    ...
    private void unenrollActivity(Request actorMessage) throws Exception {
        ...
        for (String userId : userIds) {
            ...
            activityEnrollmentDao.update(actorMessage.getRequestContext(), userId, activityId, activityType, batchId, updateData);
        ...
        if ("CF".equalsIgnoreCase(activityType) && !unenrollmentDataList.isEmpty()) {
            Map<String, Object> eventData = createBulkEnrollmentEventData(
                    batchId,
                    activityId,
                    activityType,
    ...
    private void listActivityEnrollments(Request actorMessage) throws Exception {
        ...
        List<String> activityIds = (List<String>) request.getOrDefault("activityIds", null);

        List<Map<String, Object>> enrollments = activityEnrollmentDao.listUserEnrollments(actorMessage.getRequestContext(), userId, activityIds);

        Response response = new Response();
        Map<String, Object> result = new HashMap<>();
        result.put("activities", enrollments != null ? enrollments : new java.util.ArrayList<>());
        result.put(JsonKey.COUNT, enrollments != null ? enrollments.size() : 0);
        response.put(JsonKey.RESPONSE, result);
        sender().tell(response, self());

        logger.info(actorMessage.getRequestContext(),
                "ActivityEnrolmentActor: listActivityEnrollments - Listed enrollments for userId: " + userId + ", count: " + (enrollments != null ? enrollments.size() : 0));
    }
